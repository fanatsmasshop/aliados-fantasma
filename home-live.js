...
function bindImageFallbacks(root = document) {
  const useCoverFallback = image => {
    if (image.dataset.fallbackApplied === '1') return;
    image.dataset.fallbackApplied = '1';
    image.src = DEFAULT_LOGO;
    image.classList.add('fallback');
  };
  const useLogoFallback = image => {
    if (image.dataset.fallbackApplied === '1') return;
    image.dataset.fallbackApplied = '1';
    image.src = DEFAULT_LOGO;
  };
  root.querySelectorAll('.live-slide-bg,.live-card-cover').forEach(image => {
    image.addEventListener('error', () => useCoverFallback(image), { once: true });
    if (image.complete && image.naturalWidth === 0) useCoverFallback(image);
  });
  root.querySelectorAll('.live-slide-logo,.live-card-logo').forEach(image => {
    image.addEventListener('error', () => useLogoFallback(image), { once: true });
    if (image.complete && image.naturalWidth === 0) useLogoFallback(image);
  });
}

function syncLiveLayoutState() {
  const hasBusinesses = state.businesses.length > 0;
  document.body.classList.toggle('af-live-has-businesses', hasBusinesses);
  document.body.classList.toggle('af-live-empty-network', !hasBusinesses);
  elements.live?.classList.toggle('has-businesses', hasBusinesses);
  elements.live?.classList.toggle('empty-network', !hasBusinesses);
}
...
function renderSlider() {
  ...
  if (!candidates.length) {
    elements.slider?.closest('.live-slider-wrap')?.classList.add('is-empty');
  ...
  elements.slider?.closest('.live-slider-wrap')?.classList.remove('is-empty');
...
function renderAllBusinesses() {
  ...
  elements.grid.querySelectorAll('.live-skeleton').forEach(node => node.remove());
  elements.grid.innerHTML = state.filtered.map(item => businessCardMarkup(item)).join('');
  const hasResults = state.filtered.length > 0;
  elements.empty?.classList.toggle('hidden', hasResults);
  elements.grid.classList.toggle('hidden', !hasResults);
  if (elements.empty) {
    const title = elements.empty.querySelector('h3');
    const copy = elements.empty.querySelector('p');
    if (!state.businesses.length) {
      if (title) title.textContent = 'La red está lista para crecer.';
      if (copy) copy.textContent = 'Aún no hay negocios públicos para mostrar. Puedes publicar lo que necesitas o registrar el primero.';
    } else if (state.query || state.category) {
      if (title) title.textContent = 'No encontramos coincidencias.';
      if (copy) copy.textContent = 'Prueba otra búsqueda o limpia el filtro para ver todos los negocios disponibles.';
    }
  }
...
async function loadBusinesses() {
  ...
  const businesses = (data || []).filter(item => !item.estado_operativo || item.estado_operativo === 'activo');
...
export async function activateLiveHome() {
  ...
  try {
    ...
    if (elements.grid) elements.grid.innerHTML = '<div class="live-load-error"><strong>No pudimos sincronizar el directorio.</strong><br><a href="explorar.html">Abrir directorio completo</a></div>';
    elements.featuredSection?.classList.add('hidden');
    elements.promotionsSection?.classList.add('hidden');
  } finally {
    state.loading = false;
  }
}
