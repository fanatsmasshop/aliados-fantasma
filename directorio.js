...
function applyInitialUrlFilters() {
  ...
  const regionParam = String(params.get('estado') || '').trim();
  const municipalityParam = String(params.get('municipio') || '').trim();
  ...
  if (categoryParam) {
    ...
    const category = state.categories.find(item =>
      normalize(item.slug) === target || normalize(item.nombre) === target
    );
    if (category) {
      state.category = category.nombre;
      el.category.value = category.nombre;
    }
  }

  if (regionParam) {
    const region = [...el.region.options].find(option => normalize(option.value) === normalize(regionParam));
    if (region) {
      state.region = region.value;
      el.region.value = region.value;
      populateMunicipalities();
    }
  }

  if (municipalityParam) {
    const municipality = [...el.municipality.options].find(option => normalize(option.value) === normalize(municipalityParam));
    if (municipality) {
      state.municipality = municipality.value;
      el.municipality.value = municipality.value;
    }
  }
}
...
function syncUrlFromState() {
  ...
  if (state.category) {
    const category = state.categories.find(item => item.nombre === state.category);
    params.set('categoria', category?.slug || state.category);
  }
  if (state.region) params.set('estado', state.region);
  if (state.municipality) params.set('municipio', state.municipality);
...
async function loadDirectory() {
  ...
  const { data: categories } = await supabase.from('categorias').select('nombre,icono,orden,slug').eq('activa', true).order('orden');
  state.categories = categories || [];
  populateFilters();
  applyInitialUrlFilters();
  applyFilters({ trackSearch: Boolean(state.query) });
