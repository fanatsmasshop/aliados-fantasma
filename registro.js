...
const DRAFT_KEY='af_registro_borrador_ux2';
let step=1;
let submitting=false;

const fields={responsable:'#responsable',email:'#email',password:'#password','password-confirm':'#password-confirm',negocio:'#negocio',categoria:'#categoria',whatsapp:'#whatsapp',estado_region:'#estado_region',municipio:'#municipio',terms:'#terms'};
...
function validPhone(v){const d=digits(v);return d.length===10||(d.length===12&&d.startsWith('52'));}
...
function validateStep(n){
  let ok=true;
  const req=(id,msg)=>{setError(id,'');if(!val(id)){setError(id,msg);ok=false;}};
  if(n===1){
    req('responsable','Escribe tu nombre.');req('email','Escribe tu correo.');
    if(val('email')&&!validEmail(val('email'))){setError('email','Escribe un correo válido.');ok=false;}
    setError('password','');if(!isStrongPassword(password.value)){setError('password',PASSWORD_HELP);ok=false;}
    setError('password-confirm','');if(password.value!==confirm.value){setError('password-confirm','Las contraseñas no coinciden.');ok=false;}
  }
  if(n===2){
    ['negocio','categoria','estado_region','municipio'].forEach(id=>req(id,'Este campo es obligatorio.'));
    req('whatsapp','Escribe tu WhatsApp.');
    if(val('whatsapp')&&!validPhone(val('whatsapp'))){setError('whatsapp','Escribe un número mexicano de 10 dígitos.');ok=false;}
    setError('terms','');if(!el('terms')?.checked){setError('terms','Debes aceptar los términos y el aviso de privacidad.');ok=false;}
  }
  return ok;
}
...
function go(n){
  step=n;
  document.querySelectorAll('.form-step').forEach(x=>x.classList.toggle('active',Number(x.dataset.step)===n));
  document.querySelectorAll('.auth-step').forEach(x=>{const k=Number(x.dataset.progress);x.classList.toggle('active',k===n);x.classList.toggle('done',k<n);});
  const titles={1:['Tu acceso','Primero crea tus datos de acceso.'],2:['Tu negocio','Solo necesitamos lo esencial. El resto lo completas dentro del panel.']};
  document.querySelector('#step-title').textContent=titles[n][0];
  document.querySelector('#step-subtitle').textContent=titles[n][1];
  document.querySelector('.register-card')?.scrollIntoView({behavior:'smooth',block:'start'});
}
function saveDraft(){const data={};['responsable','email','negocio','categoria','whatsapp','estado_region','municipio'].forEach(id=>data[id]=val(id));localStorage.setItem(DRAFT_KEY,JSON.stringify(data));}
...
document.querySelectorAll('[data-next]').forEach(x=>x.addEventListener('click',()=>{if(validateStep(step)){saveDraft();go(2);}}));
document.querySelectorAll('[data-prev]').forEach(x=>x.addEventListener('click',()=>go(1)));

form?.addEventListener('submit',async event=>{
  event.preventDefault();
  if(submitting||!validateStep(2)||!supabase)return;
  ...
  try{
    ...
    const {data,error}=await supabase.auth.signUp({
      ...
      options:{emailRedirectTo:redirect,data:{
    ...
    }else location.replace('registro-enviado.html');
  }catch(error){
    console.error(error);
  ...
  }finally{submitting=false;button.disabled=false;setButtonText('Crear cuenta y verificar');}
});
